...
3. Automação do navegador: login (perfil Edge opcional), busca do PO, download
   dos anexos.
4. Pós-processamento: usa uma pasta canônica `PO_<numero>__WORK` durante a coleta
  e só no final faz um rename atômico para `PO_<numero>_<STATUS>` (`_COMPLETED`, `_FAILED`, `_PARTIAL`, `_NO_ATTACHMENTS`).
  Agora sempre existe apenas **uma** pasta final por PO.
