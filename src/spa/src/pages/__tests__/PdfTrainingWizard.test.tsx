...
vi.mock('../../components/DocumentProcessingStatus', () => ({
  default: () => <div>ProcessingStatus</div>
}));
...
vi.mock('../../components/LLMSupportPanel', () => ({
  default: ({ documentId }: { documentId: string | null }) => (
    <div data-testid="llm-support-prop">{documentId ?? 'none'}</div>
  )
}));
...
describe('PdfTrainingWizard integration', () => {
  it('passes the selected document id to the PdfViewer', () => {
    ...
    expect(screen.getByTestId('llm-support-prop')).toHaveTextContent('doc-123');
  });
});
