...
class ExcelProcessor:
    ...
    - Honors central config: Config.EXCEL_FILE_PATH when set
    ...
    - Falls back to `<project_root>/data/input/input.xlsx`
        ...
        if Config.EXCEL_FILE_PATH:
            return os.path.expanduser(Config.EXCEL_FILE_PATH)
    ...
    def get_progress_file_path() -> str:
        """Get the full path to the progress spreadsheet file (separate from input).

    This file is used for updating progress and can be modified without affecting the input file.
        """
        # Use the same logic as input file but with _progress suffix
        input_path = ExcelProcessor.get_excel_file_path()
        base, ext = os.path.splitext(input_path)
        progress_path = f"{base}_progress{ext}"
        
        # If progress file doesn't exist, copy from input file
        if not os.path.exists(progress_path):
            import shutil
            shutil.copy2(input_path, progress_path)
            print(f"📋 Created progress file: {os.path.basename(progress_path)}")
    ...
    def read_po_numbers_from_excel(self, excel_file_path: str) -> Tuple[List[Dict[str, Any]], List[str], List[str], bool]:
        ...
        try:
            ...
            # Clean column names by stripping BOM characters
            df.columns = [self._strip_bom_from_column_name(col) for col in df.columns]
            ...
            for _, row in df_processed.iterrows():
                ...
                po_entry = {
                    ...
                    'attachments_found': self._safe_int(row.get('ATTACHMENTS_FOUND', 0)),
                    'attachments_downloaded': self._safe_int(row.get('ATTACHMENTS_DOWNLOADED', 0)),
    ...
    def _read_csv_auto(path: str) -> Tuple[pd.DataFrame, str, str]:
        ...
        """
        # Try different encodings, starting with the most likely to work
        encodings = ["utf-8", "utf-8-sig", "latin1", "cp1252", "iso-8859-1", "windows-1252"]
        last_err: Exception | None = None
        for enc in encodings:
            try:
                # First pass: detect delimiter from header
                with open(path, 'r', encoding=enc, errors='ignore') as f:
                    header_line = f.readline()
                detected_sep = ';' if header_line.count(';') > header_line.count(',') else ','
                
                # Second pass: read with explicit configuration to prevent indexing issues
                df = pd.read_csv(
                    path, 
                    sep=detected_sep, 
                    encoding=enc, 
                    index_col=False,    # Explicitly prevent using first column as index
                    header=0,           # First line is header
                    dtype=str,          # Force all as string to avoid type issues
                    na_filter=False     # Prevent NaN conversion for empty values
                )
    ...
    def _safe_int(value, default=0):
        """Safely convert a value to int, handling empty strings and non-numeric values."""
    ...
    @staticmethod
    def _strip_bom_from_column_name(col_name: str) -> str:
        """Strip BOM characters from column name."""
        bom_chars = ['\ufeff', '\u00ef\u00bb\u00bf']
        cleaned = str(col_name)
        for bom in bom_chars:
            cleaned = cleaned.replace(bom, '')
        return cleaned
        """Find a column by name, ignoring case, surrounding spaces, and BOM characters."""
        if target in df.columns:
            return target
        
        # Handle BOM characters that might be present in column names
        bom_chars = ['\ufeff', '\u00ef\u00bb\u00bf']  # UTF-8 BOM and its escaped representation
        
        # Create mapping with cleaned column names
        low = {}
        for c in df.columns:
    ...
    def process_po_numbers(po_entries: List[Dict[str, Any]]) -> List[Tuple[str, str]]:
        ...
        for entry in po_entries:
            ...
            if clean_po.isdigit() and len(clean_po) >= 6:
                ...
                # Removed verbose PO listing to reduce terminal output clutter
                # print(f"  ✅ {display_po} → Will process")
            else:
                print(f"  ❌ Invalid PO format: {po_number}")
                # Update status to FAILED (skip CSV writes when SQLite-only persistence is enabled)
                if not Config.SQLITE_ONLY_PERSISTENCE:
    ...
    def update_po_status(po_number: str, status: str, supplier: str = '', 
        ...
        """
        excel_file_path = ExcelProcessor.get_progress_file_path()
        _, ext = os.path.splitext(excel_file_path.lower())
        ...
        for attempt in range(max_retries):
            try:
                ...
                with open(lock_file, 'w') as lock_f:
                    ...
                    ExcelProcessor._update_po_status_locked(
                        excel_file_path, ext, po_number, status, supplier,
                        attachments_found, attachments_downloaded, error_message,
                        download_folder, coupa_url, attachment_names
                    )
                    
                    # Release lock (automatically released when file is closed)
                    fcntl.flock(lock_f.fileno(), fcntl.LOCK_UN)
                    
                # Clean up lock file if we're the last one
                try:
                    os.remove(lock_file)
                except:
                    pass
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"❌ Error updating PO status: Could not acquire file lock after {max_retries} attempts")
                    return
            except Exception as e:
    ...
    @staticmethod
    def _update_po_status_locked(excel_file_path: str, ext: str, po_number: str, status: str, 
                                supplier: str, attachments_found: int, attachments_downloaded: int,
                                error_message: str, download_folder: str, coupa_url: str, 
                                attachment_names: str) -> None:
        """Update PO status with file lock already acquired."""
        try:
            if ext == ".csv":
                # Create automatic backup before any modifications
                backup_path = ExcelProcessor._create_auto_backup(excel_file_path)
                if backup_path:
                    print(f"🛡️ Auto-backup created: {os.path.basename(backup_path)}")
                
                # Update CSV by reading (auto), modifying the row, and writing back preserving delimiter.
                # Use UTF-8 without BOM for better compatibility with text editors
                ...
                # Only update existing columns - do not add new ones to preserve input file structure
                ...
                # Strip BOM characters from data values to ensure proper matching
                bom_chars = ['\ufeff', '\u00ef\u00bb\u00bf']
                def _strip_bom(text: str) -> str:
                    for bom in bom_chars:
                        if text.startswith(bom):
                            return text[len(bom):]
                    return text
                series = series.apply(_strip_bom)
                ...
                if 'ATTACHMENTS_FOUND' in df.columns:
                    df.loc[mask, 'ATTACHMENTS_FOUND'] = attachments_found
                if 'ATTACHMENTS_DOWNLOADED' in df.columns:
                ...
                if 'DOWNLOAD_FOLDER' in df.columns:
                    df.loc[mask, 'DOWNLOAD_FOLDER'] = download_folder
                if 'COUPA_URL' in df.columns:
                ...
                # Write back with stable settings for Excel compatibility (matches src/ implementation)
                # Use a more robust approach to prevent file corruption
                ...
                try:
                    df.to_csv(
                        ...
                        lineterminator="\n",
                        quoting=csv.QUOTE_ALL,  # Use QUOTE_ALL for better compatibility
                        escapechar='\\',  # Add escape character for special chars
                        errors='replace'  # Replace problematic characters instead of failing
                    )
                    
                    # Verify the temp file was written correctly
                    temp_df, _, _ = ExcelProcessor._read_csv_auto(temp_file)
                    if len(temp_df) == len(df):
                        # Only replace original if temp file is valid
                        import shutil
                        shutil.move(temp_file, excel_file_path)
                        print(f"💾 CSV updated successfully: {len(df)} rows written")
                    else:
                        raise Exception(f"Temp file validation failed: expected {len(df)} rows, got {len(temp_df)}")
                    ...
                    if os.path.exists(temp_file):
                        try:
                            ...
                            pass
                    raise Exception(f"Failed to write CSV: {write_error}")
            else:
                ...
                for row_num in range(2, worksheet.max_row + 1):  # Start from row 2 (skip header)
                    ...
                    if str(cell_value).strip() == str(po_number).strip():
                        ...
                        for col, value in [
                            (2, status),  # STATUS column
                            (3, supplier),  # SUPPLIER column
                            (4, attachments_found),  # ATTACHMENTS_FOUND column
                            (5, attachments_downloaded),  # ATTACHMENTS_DOWNLOADED column
                            (6, attachment_names),  # AttachmentName column
                            (7, datetime.now().strftime('%Y-%m-%d %H:%M:%S')),  # LAST_PROCESSED column
                            (8, error_message),  # ERROR_MESSAGE column
                            (9, download_folder),  # DOWNLOAD_FOLDER column
                            (10, coupa_url),  # COUPA_URL column
                        ]:
                            cell = worksheet.cell(row=row_num, column=col)
                            # Handle merged cells by assigning to the top-left cell of the merge range
    ...
    @staticmethod
    def _find_column(df: pd.DataFrame, column_name: str) -> Optional[str]:
        """Find a column in the DataFrame by name (case-insensitive).
        ...
        Returns:
            The exact column name if found, else None.
        """
    ...
    @staticmethod
    def _create_auto_backup(file_path: str) -> Optional[str]:
        """Create an automatic backup of the file before modifications.
        ...
        try:
            ...
            backup_filename = f"{name}_auto_{timestamp}{ext}"
            ...
            print(f"⚠️ Failed to create auto-backup: {e}")
            return None
