...
class POListRenderer:
    ...
    @staticmethod
    def build_label(record: PORecordView, index: Optional[int] = None, total: Optional[int] = None) -> Text:
...
class POListView(Tree[None]):
    ...
    def __init__(self) -> None:
        ...
        self._po_nodes: Dict[str, TreeNode] = {}

    @property
    def po_nodes(self) -> Dict[str, TreeNode]:
        """Safely access the _po_nodes dictionary."""
        if not isinstance(self._po_nodes, dict):
            # Defensive: if _po_nodes got corrupted, reset it
            self._po_nodes = {}
        return self._po_nodes

    def __getattr__(self, name: str):
        """Defensive getattr to handle Textual library accessing _nodes."""
        if name == '_nodes':
            # If Textual is looking for _nodes, return our _po_nodes
            return self.po_nodes
        # Default behavior
    ...
    def update_from_snapshot(self, snapshot: ProgressSnapshot) -> None:
        ...
        for po_number in list(self._po_nodes.keys()):
            if po_number not in desired:
                node = self._po_nodes.pop(po_number)
                node.remove()

        # Update / insert nodes with index
        total = len(snapshot.records)
        for index, record in enumerate(snapshot.records, 1):
            node = self._po_nodes.get(record.po_number)
            label = POListRenderer.build_label(record, index, total)
            if node is None:
                node = self.root.add(label, data=None)
                node.allow_expand = True
                self._po_nodes[record.po_number] = node
            else:
...
class DownloadCLIController:
    ...
    def stop(self) -> None:
        ...
        if hasattr(self, '_ui_thread') and self._ui_thread:
    ...
    def run_in_background(
        ...
        *,
        ...
        """Run the Textual app in background thread while processing continues in foreground.

        This method starts the UI in a separate thread and returns immediately,
        allowing the main processing to continue without blocking.
