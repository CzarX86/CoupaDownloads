# Plan: Bootstrapping Coupa Turbo Downloader (Independent Standalone App)

Este plano descreve as etapas técnicas detalhadas para criar, estruturar, implementar e empacotar a nova aplicação portátil **Coupa Turbo Downloader** na pasta isolada `/CoupaTurboDownloader`.

## User Review Required

> [!IMPORTANT]
> **Isolamento Total & Referência Estrita**: A nova aplicação viverá exclusivamente dentro da pasta `/CoupaTurboDownloader` no repositório. O ambiente Python será gerenciado de forma independente via `uv` com suas próprias dependências leves.
> 
> **IMPORTANTE**: O projeto legado em `src/` ou `tools/` **nunca** será alterado, modificado ou copiado. Ele servirá puramente como referência conceitual e de regras de negócio. Todo o desenvolvimento do `CoupaTurboDownloader` será construído totalmente do zero, sem copiar ou colar arquivos do projeto original.
> 
...
### [NEW] [CoupaTurboDownloader Directory Structure](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader)

Toda a infraestrutura do subprojeto será construída sob este novo diretório.

#### [NEW] [pyproject.toml](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/pyproject.toml)
- Definição do pacote independente Python 3.12.
- Dependências enxutas: `pywebview`, `httpx`, `beautifulsoup4`, `lxml`, `pyinstaller`.

#### [NEW] [config.json](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/.planning/config.json)
- Configurações do GSD específicas para este subprojeto (YOLO mode, Balanced AI profile, auto-advance).

#### [NEW] [PROJECT.md](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/.planning/PROJECT.md)
- Contexto de valor e metas (69.44 POs/m, zero-browser, multiplataforma).

#### [NEW] [REQUIREMENTS.md](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/.planning/REQUIREMENTS.md)
...
#### [NEW] [ROADMAP.md](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/.planning/ROADMAP.md)
...
#### [NEW] [index.html](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/src/gui/assets/index.html)
...
#### [NEW] [style.css](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/src/gui/assets/style.css)
- Tokens de design: Cores escuras HSL/RGB, efeitos de desfoque de fundo (`backdrop-filter`), transições e estilos responsivos.

#### [NEW] [script.js](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/src/gui/assets/script.js)
...
#### [NEW] [api.py](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/src/gui/api.py)
...
#### [NEW] [crawler.py](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/src/engine/crawler.py)
...
#### [NEW] [parser.py](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/src/engine/parser.py)
...
#### [NEW] [authenticator.py](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/src/engine/authenticator.py)
- Selenium Edge WebDriver temporário para extração automática de cookies de login.

#### [NEW] [benchmarker.py](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/src/engine/benchmarker.py)
- Testes concorrentes de ping e pacotes para calcular e sugerir parâmetros ideais baseados na rota local.

#### [NEW] [rate_limiter.py](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/src/engine/rate_limiter.py)
- Controle de tráfego: delays controlados (0.03s) e janelas de cooldown para lotes superiores a 500 POs.

#### [NEW] [updater.py](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/src/engine/updater.py)
...
#### [NEW] [session_db.py](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/src/db/session_db.py)
...
#### [NEW] [main.py](file:///Users/juliocezar/Dev/CoupaTurboDownloader/src/main.py)
- Entrypoint principal do aplicativo executável.

#### [NEW] [build_standalone.py](file:///Users/juliocezar/Dev/CoupaPilot/CoupaTurboDownloader/build_standalone.py)
...
### Automated Tests
- Criaremos uma bateria de testes unitários isolados sob `CoupaTurboDownloader/tests/` usando `pytest` e `pytest-asyncio` para cobrir:
