...
from EXPERIMENTAL.csv_handler import CSVHandler, WriteQueue
...
def process_po_worker(args):
    ...
    if csv_path:
        try:
            from pathlib import Path
            from EXPERIMENTAL.csv_handler import CSVHandler
...
def _interactive_setup() -> 'InteractiveSetupSession':
    ...
    if workers < 1:
        workers = 1
    ExperimentalConfig.USE_PROCESS_POOL = workers > 1
    ExperimentalConfig.PROC_WORKERS = workers if workers > 1 else 1
    # Treat cap as unlimited unless explicitly set elsewhere
    if getattr(ExperimentalConfig, 'PROC_WORKERS_CAP', None) is None:
        ExperimentalConfig.PROC_WORKERS_CAP = 0
    mode_label = "parallel" if ExperimentalConfig.USE_PROCESS_POOL else "sequential"
...
def _apply_env_overrides() -> Dict[str, Any]:
    ...
    try:
        ...
        workers = 2
    ExperimentalConfig.PROC_WORKERS = max(1, workers)
    ExperimentalConfig.USE_PROCESS_POOL = ExperimentalConfig.PROC_WORKERS > 1
    cap_env = os.environ.get('PROC_WORKERS_CAP')
    if cap_env is not None:
        try:
            ExperimentalConfig.PROC_WORKERS_CAP = int(cap_env)
        except ValueError:
            ExperimentalConfig.PROC_WORKERS_CAP = 0
    if getattr(ExperimentalConfig, 'PROC_WORKERS_CAP', None) is None:
        ExperimentalConfig.PROC_WORKERS_CAP = 0
