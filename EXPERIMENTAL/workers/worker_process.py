...
from EXPERIMENTAL.corelib.browser import BrowserManager
from EXPERIMENTAL.corelib.downloader import Downloader
from EXPERIMENTAL.corelib.folder_hierarchy import FolderHierarchyManager
from EXPERIMENTAL.corelib.po_processing import process_single_po  # shared utility
from EXPERIMENTAL.corelib.driver_manager import DriverManager

# Centralized config for URLs
try:
    from EXPERIMENTAL.corelib.config import Config as _ExperimentalConfig  # type: ignore
...
class WorkerProcess:
    ...
    def _apply_download_root_override(self) -> str:
        ...
        if not target:
            try:
                from EXPERIMENTAL.corelib.config import Config as _Cfg
                target = getattr(_Cfg, 'DOWNLOAD_FOLDER', None)
            except Exception:
                try:
                    from corelib.config import Config as _Cfg  # type: ignore
                    target = getattr(_Cfg, 'DOWNLOAD_FOLDER', None)
                except Exception:
                    pass
        ...
        try:
            from EXPERIMENTAL.corelib import config as experimental_config
