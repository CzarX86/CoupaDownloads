...
## Considerações de Empacotamento (Executável Futuro)
- **Dependências**: Use apenas bibliotecas built-in do Python (Tkinter, threading, queue, json, os, sys) para garantir compatibilidade com PyInstaller/cx_Freeze.
- **Arquivos externos**: Evite dependências de arquivos externos ou binários que compliquem o bundling.
- **Processos**: Mantenha isolamento de processos para suportar empacotamento em executável standalone.
- **Configuração**: Use caminhos padrão do sistema operacional para arquivos de configuração (diretório do usuário).
- **Interface**: UI em Tkinter garante compatibilidade cross-platform sem dependências externas.
