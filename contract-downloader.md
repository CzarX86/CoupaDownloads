# Plan: Contract Downloader

Este plano detalha as fases técnicas de desenvolvimento, arquitetura de componentes, dependências de tarefas e critérios de verificação para a criação da aplicação portátil **Contract Downloader** na pasta isolada `/ContractDownloader`.
...
- **Task 1.1**: Setup do diretório e ambiente `uv`
  ...
  - **INPUT**: Pasta vazia `/ContractDownloader`
...
- **Task 4.2**: Automação de Builds Standalone Portáteis (PyInstaller)
  ...
  - **OUTPUT**: Script de build automatizado que executa o PyInstaller com parâmetros `--onefile --noconsole --clean --add-data "src/gui/assets;src/gui/assets"` (ajustando a sintaxe de barras por OS), gerando o executável final standalone de arquivo único sob `/ContractDownloader/dist`.
...
- [ ] **Security Scan**:
  ```bash
  python .agent/skills/vulnerability-scanner/scripts/security_scan.py ContractDownloader
  ```
- [ ] **Automated Unit Tests**:
  ```bash
  uv run pytest ContractDownloader/tests/
  ```
- [ ] **Manual Portability & Build Verification**:
  ```bash
  python ContractDownloader/build_standalone.py
  ```
