...
class BaseTabManager(ITabManager):
    ...
    def __init__(self, driver: webdriver.Edge, window_ids: List[str]):
        ...
        # Configurações - Usar o mesmo seletor do sistema principal
        self.attachment_selector = "span[aria-label*='file attachment'], span[role='button'][aria-label*='file attachment'], span[title*='.pdf'], span[title*='.docx'], span[title*='.msg']"
        self.allowed_extensions = [".pdf", ".msg", ".docx", ".xlsx", ".txt", ".jpg", ".png", ".zip", ".rar", ".csv", ".xml"]
        
        # Melhorias implementadas - Gestão de foco e controle de janelas
        self.max_tabs_per_window = 2  # Reduzido para melhor gestão
        self.window_focus_tracker: Dict[str, float] = {}  # Rastrear último uso de cada janela
        self.focus_timeout = 0.1  # Tempo para garantir foco
        self.window_check_interval = 5.0  # Verificar janelas a cada 5 segundos
        
        # Inicializar tracker de foco para todas as janelas
        for window_id in self.window_ids:
            self.window_focus_tracker[window_id] = time.time()
    
    def _ensure_window_focus(self, window_id: str, window_name: str) -> bool:
        ...
        try:
            ...
            if current_window != window_id:
                print(f"⚠️ [{threading.current_thread().name}] Falha ao focar {window_name}")
                return False
            
            print(f"🎯 [{threading.current_thread().name}] Focando em {window_name} (ID: {window_id[:8]}...)")
            
            # Atualizar tracker de foco
            self.window_focus_tracker[window_id] = time.time()
            
            # Pequeno delay para garantir que o foco foi aplicado
            time.sleep(self.focus_timeout)
            return True
    ...
    def _is_window_active(self, window_id: str) -> bool:
        ...
        try:
            ...
            # Verificar se a janela não está muito tempo sem uso
            last_use = self.window_focus_tracker.get(window_id, 0)
            if time.time() - last_use > self.window_check_interval:
                print(f"⚠️ [{threading.current_thread().name}] Janela {window_id[:8]}... não usada há muito tempo")
                return False
            
            return True
            
        except Exception as e:
            print(f"⚠️ [{threading.current_thread().name}] Janela {window_id[:8]}... não está ativa: {e}")
            return False
    
    def _get_best_window(self, url_index: int) -> tuple:
        """Seleciona a melhor janela para processar baseada em carga e foco."""
        with self.lock:
            # Calcular carga de cada janela
            window_loads = {}
            for window_id in self.window_ids:
                tab_count = len([tab for tab in self.active_tabs.values() if tab.window_id == window_id])
                last_use = self.window_focus_tracker.get(window_id, 0)
                time_since_use = time.time() - last_use
                
                # Priorizar janelas com menos abas e mais recentemente usadas
                load_score = tab_count + (time_since_use / 10.0)
                window_loads[window_id] = load_score
            
            # Selecionar janela com menor carga
            best_window = min(window_loads.keys(), key=lambda w: window_loads[w])
            window_index = self.window_ids.index(best_window)
            window_name = f"Janela {window_index + 1}"
            
            return best_window, window_name
    
    def create_tab_for_url(self, url_index: int, url: str, 
                          window_id: str, window_name: str) -> Optional[TabInfo]:
        """Cria uma aba para uma URL específica com gestão de foco otimizada."""
        try:
            # Garantir foco na janela antes de criar aba
            if not self._ensure_window_focus(window_id, window_name):
                return None
            
            # Verificar limite de abas por janela
            with self.lock:
                current_tabs = len([tab for tab in self.active_tabs.values() if tab.window_id == window_id])
                if current_tabs >= self.max_tabs_per_window:
                    print(f"⚠️ [{threading.current_thread().name}] {window_name} atingiu limite de {self.max_tabs_per_window} abas")
                    return None
                
                # Mudar para a janela especificada (já focada)
                self.driver.switch_to.window(window_id)
...
class FIFOTabManager(BaseTabManager):
    ...
    def _ensure_window_focus(self, window_id: int, window_name: str):
        """Garante que a janela está focada antes de processar."""
        try:
            # Focar na janela específica
            self.driver.switch_to.window(window_id)
    ...
    def _is_window_active(self, window_id: int) -> bool:
        """Verifica se a janela está ativa e responsiva."""
        try:
            # Tentar focar na janela
            self.driver.switch_to.window(window_id)
    ...
    def process_url_with_inventory(self, url_index: int, url: str, max_workers: int) -> Dict[str, Any]:
        ...
        try:
            # Determinar qual janela usar (distribuir entre janelas com controle de foco)
    ...
    def get_window_status(self) -> Dict[str, Any]:
        """Retorna status detalhado das janelas com melhorias implementadas."""
        with self.lock:
            status = {
                'total_windows': len(self.window_ids),
                'active_windows': 0,
                'total_tabs': len(self.active_tabs),
                'max_tabs_per_window': self.max_tabs_per_window,
                'window_details': []
            }
            ...
            for i, window_id in enumerate(self.window_ids):
                ...
                is_active = self._is_window_active(window_id)
                if is_active:
                    status['active_windows'] += 1
                
                status['window_details'].append({
                    'window_id': window_id[:8] + "...",
                    'window_name': f"Janela {i+1}",
                    'tab_count': tab_count,
                    'is_active': is_active,
                    'time_since_use': round(time_since_use, 2),
                    'load_percentage': round((tab_count / self.max_tabs_per_window) * 100, 1)
                })
...
class ParallelTabManager(BaseTabManager):
    ...
    def process_url_with_inventory(self, url_index: int, url: str, max_workers: int) -> Dict[str, Any]:
        ...
        try:
            # Selecionar melhor janela baseada em carga e foco
            window_id, window_name = self._get_best_window(url_index)
            
            # Garantir que a janela está focada antes de criar aba
            if not self._ensure_window_focus(window_id, window_name):
                result['error'] = f"Falha ao focar {window_name}"
                return result
